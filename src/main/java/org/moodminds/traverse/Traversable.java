...
public interface Traversable<V, E extends Exception> extends TraverseSupport<V, E> {
    ...
    /**
     * Return a reducing {@link Resolver} using the provided {@link Collector}.
     * <p>
     * In non explicitly sequential traversals with the non-({@link Collector.Characteristics#CONCURRENT concurrent})
     * {@link Collector} before combining the accumulated intermediate results ({@link A}) in the combination
     * stage (of a {@link Collector#combiner()}), the algorithm sorts these results based on the completion
     * time of the worker {@link Thread}. The accumulated result of the last completed {@link Thread} will
     * be combined last. This reasonably ensures (not 100%, but proportionate to the number of worker
     * {@link Thread}s) that the last combined intermediate result ({@link A}) corresponds to the last
     * emitted element - i.e., that the element was accumulated in that result ({@link A}). This is
     * useful in cases of searching for the last emitted element with a reduction function.
     *
     * @param collector the specified {@link Collector} collector
     * @param <V>       the type of item values
     * @param <E>       the type of traversal exception
     * @param <A>       the result container type of the specified {@link Collector} collector
     * @param <R>       the type of the collecting result
     * @param <H>       the type of resolution exception
     * @return a reducing {@link Resolver} with the given {@link Collector} collector
     * @throws NullPointerException if the given {@link Collector} collector is {@code null}
     */
    static <V, E extends Exception, A, R, H extends Exception> Resolver<V, E, R, H> reduce(Collector<? super V, A, ? extends R> collector) {
